...
export default function ChatInterface({ apiKey, onReset }: ChatInterfaceProps) {
  ...
  const stepCount = Math.min(Math.floor(displayMessages.length / 2), 6)
          ...
          <div className="flex items-center gap-2">
            <div className="flex gap-1">
              {[1, 2, 3, 4, 5, 6].map((step) => (
                <div
                  key={step}
                  className={'h-0.5 w-6 rounded-full transition-colors ' + (stepCount >= step ? 'bg-accent' : 'bg-border')}
                />
              ))}
            </div>
            <span className="text-muted text-xs font-mono">{stepCount}/6</span>
          </div>
        </div>
      )}
      ...
      {sessionComplete && (
        <div className="border-t border-border px-6 py-4 flex-shrink-0">
          <div className="max-w-2xl mx-auto flex items-center justify-between">
            <p className="text-muted text-sm font-body">Runbook complete. You're ready.</p>
            <button
              onClick={handleNewSession}
